          ...
          <div className='flex flex-col sm:flex-row items-center md:justify-between justify-center gap-4'>
              {
                categories.map((category,index)=>(
                  ...
                  key={index} className={`py-1 sm:px-5  px-8 rounded-full hover:bg-primary text-[#1e1e1e] font-semibold text-[18px] hover:text-white transition-colors font-poppins cursor-pointer ${selectedCategory === category ? 'bg-white text-primary' : 'text-secondary'}`}>{category}</button>
                ))
              }
          </div>
        </div>
