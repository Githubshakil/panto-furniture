...
const NavbarSection = () => {
    const [isMenuOpen, setIsMenuOpen] = useState(false); // Placeholder for future state management
  ...
  return (
    <header>
      <nav className="max-w-screen-2xl container mx-auto flex justify-between items-center py-6 px-4 ">
        ...
        <div onClick={toggleMenu} className="md:hidden cursor-pointer text-xl hover:text-primary transition duration-300 ">
            {
        ...
        {/* mobile menu items */}
        <div className={`fixed top-0 left-0 w-full h-screen bg-black/80 text-white flex flex-col items-center justify-center space-y-8 transition-transform duration-300 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"} md:hidden`}>
